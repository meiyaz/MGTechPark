...
const Packages = () => (
  <Section id="packages">
    <div className="text-center">
      <h2 className="text-4xl font-bold mb-12 text-foreground">Our Packages</h2>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {packages.map((pkg, index) => (
          <div key={index} className={`bg-card p-8 rounded-lg shadow-lg flex flex-col ${pkg.popular ? 'border-2 border-primary' : ''}`}>
            {pkg.popular && <p className="text-sm font-bold text-primary mb-2 self-center">MOST POPULAR</p>}
            <h3 className="text-3xl font-bold mb-4 text-foreground">{pkg.name}</h3>
            <p className="text-4xl font-extrabold mb-6 text-primary">{pkg.price}</p>
